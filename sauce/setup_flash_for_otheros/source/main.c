...
static int setup_flash(void)
{
	...
	*ptr++ = FLASH_REGION_ACL;
	...
	PRINTF("%s:%d: FLASH 7th region old start_sector (0x%016llx) new start_sector (0x%016llx)\n", __func__, __LINE__, *ptr, FLASH_7TH_REGION_NEW_START_SECTOR);

	PRINTF("%s:%d: FLASH 7th region old sector_count (0x%016llx) new sector_count (0x%016llx)\n", __func__, __LINE__, *(ptr + 1), FLASH_7TH_REGION_NEW_SECTOR_COUNT);
