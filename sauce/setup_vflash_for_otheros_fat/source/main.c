...
#define VFLASH_6TH_REGION_NEW_START_SECTOR							0x73a00ull
#define VFLASH_6TH_REGION_NEW_SECTOR_COUNT							0x4200ull

#define VFLASH_PARTITION_TABLE_7TH_REGION_OFFSET				0x300ull
#define VFLASH_7TH_REGION_NEW_START_SECTOR							0x77c00ull
#define VFLASH_7TH_REGION_NEW_SECTOR_COUNT							0x200ull
...
static int setup_vflash(void){
	...
	/* patch VFLASH 6th region */

	ptr = (uint64_t *) (buf + VFLASH_PARTITION_TABLE_6ND_REGION_OFFSET);

	*ptr++ = VFLASH_6TH_REGION_NEW_START_SECTOR;
	*ptr++ = VFLASH_6TH_REGION_NEW_SECTOR_COUNT;
	*ptr++ = VFLASH_REGION_LPAR_AUTH_ID;
	*ptr++ = VFLASH_REGION_ACL;

	/* patch VFLASH 7th region */

	ptr = (uint64_t *) (buf + VFLASH_PARTITION_TABLE_7TH_REGION_OFFSET);

	*ptr++ = VFLASH_7TH_REGION_NEW_START_SECTOR;
	*ptr++ = VFLASH_7TH_REGION_NEW_SECTOR_COUNT;
	*ptr++ = VFLASH_REGION_LPAR_AUTH_ID;
	*ptr++ = VFLASH_REGION_ACL;
